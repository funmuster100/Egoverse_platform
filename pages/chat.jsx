...
const send = async () => {
  ...
  const remember = (text) => {
    ...
    egoProfile.learningJournal.push({ text, date: new Date().toISOString() });
  ...
  const handleBrandingColorChange = (e) => setBrandingColor(e.target.value);

  return (
    <>
      <div className={styles["chat-container"]} style={{ borderTopColor: brandingColor }}>
        {/* Header */}
        <div className={styles["chat-header"]} style={{ borderBottomColor: brandingColor }}>
          <div className={styles["chat-header-left"]}>
            {brandingLogo && (
              <img src={brandingLogo} alt="Branding Logo" className={styles["branding-logo"]} />
            )}
            ...
            <div>
              ...
              <div className={styles["chat-status"]}>
                <span className={styles["status-dot"]} style={{ background: brandingColor }} /> Online
              </div>
            </div>
          </div>
          ...
          {messages.map((m, i) => (
            <div key={i} className={`${styles["bubble-container"]} ${styles[m.role]}`}>
              ...
              <div className={styles["bubble"]}>
                {m.content}
                {m.role === "assistant" && (
                  <button onClick={() => remember(m.content)} className={styles["remember-button"]}>
                    ⭐ merken
                  </button>
                )}
              </div>
